 ...
 * @returns {{ name: string, surname: string, middlename: string, birth: string, start: string, facility: string }} Объект студента
 */
function makeStudentFromData(data) {
  ...
  if (!student.birth) errors.push({ field: 'birth', message: 'Не указана дата рождения' });
  if (!student.start) errors.push({ field: 'start', message: 'Не указано начало обучения' });
  if (!student.facility) errors.push({ field: 'facility', message: 'Не указан факультет' });
 ...
 * @returns {{ id: string, name: string, surname: string, middlename: string, birth: string, start: string, facility: string }[]} Массив студентов
 */
 ...
 * @returns {{ id: string, name: string, surname: string, middlename: string, birth: string, start: string, facility: string, createdAt: string, updatedAt: string }} Объект студента
 */
 ...
 * @param {{ name?: string, surname?: string, middlename?: string, birth?: string, start?: string, facility?: string }} data - Объект с изменяемыми данными
 * @throws {ApiError} Студент с таким ID не найден (statusCode 404)
 * @throws {ApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ id: string, name: string, surname: string, middlename: string, birth: string, start: string, facility: string, createdAt: string, updatedAt: string }} Объект студента
 */
...
module.exports = createServer(async (req, res) => {
  ...
  .on('listening', () => {
    if (process.env.NODE_ENV !== 'test') {
      ...
      console.log(`POST ${URI_PREFIX} - создать студента, в теле запроса нужно передать объект { name: string, surname: string, middlename: string, birth: string, start: string, facility: string}`);
      console.log(`GET ${URI_PREFIX}/{id} - получить студента по его ID`);
      console.log(`PATCH ${URI_PREFIX}/{id} - изменить студента с ID, в теле запроса нужно передать объект { name?: string, surname?: string, middlename?: string, birth?: string, start?: string, facility?: string}`);
